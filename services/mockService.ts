...
// === SAFETY DATA SCRAPING (RESTORED FROM V1) ===
...
// === MAIN SCRAPER (OPTIMIZED FOR SPEED) ===
...
export const scrapeRealCarrier = async (mcNumber: string, useProxy: boolean): Promise<CarrierData | null> => {
  ...
  // Fetch email and safety data in PARALLEL (not sequential)
  if (carrier.dotNumber) {
    const [emailRes, safetyRes] = await Promise.all([
      fetchCarrierEmailFromSMS(carrier.dotNumber, useProxy),
      fetchSafetyData(carrier.dotNumber)
    ]);
    
    carrier.email = emailRes.replace(/Â|\[|\]/g, '').trim();
    if (carrier.email.toLowerCase().includes('email protected')) {
      carrier.email = '';
    }
    
    carrier.safetyRating = safetyRes.rating;
    carrier.safetyRatingDate = safetyRes.ratingDate;
    carrier.basicScores = safetyRes.basicScores;
    carrier.oosRates = safetyRes.oosRates;
  }
...
export const downloadCSV = (data: CarrierData[]) => {
  const headers = ['MC', 'DOT', 'Legal Name', 'Email', 'Phone', 'Status', 'Physical Address', 'MCS-150 Date', 'Safety Rating', 'BASIC Scores'];
  const csvRows = data.map(row => {
    const basicScoresStr = row.basicScores?.map(s => `${s.category}:${s.measure}`).join('; ') || '';
    return [
      row.mcNumber,
      row.dotNumber,
...
export const generateMockCarrier = (mc: string, b: boolean): CarrierData => ({
  ...
  basicScores: [
    { category: 'Unsafe Driving', measure: '45.2' },
    { category: 'HOS Compliance', measure: '52.1' },
    { category: 'Vehicle Maintenance', measure: '38.9' },
    { category: 'Driver Fitness', measure: '41.3' }
  ],
  oosRates: [
    { type: 'Vehicle', rate: '18.5%', nationalAvg: '21.4%' },
    { type: 'Driver', rate: '4.2%', nationalAvg: '5.5%' },
