...
// ... US_STATES, OPERATION_CLASSIFICATIONS, CARRIER_OPERATIONS, CARGO_TYPES constants same as before ...
  ...
  <div className="absolute z-[110] bottom-full left-1/2 -translate-x-1/2 mb-3 w-72 bg-slate-950 border-2 border-slate-700 p-4 rounded-2xl shadow-2xl animate-in fade-in zoom-in-95 slide-in-from-bottom-2">
    ...
    <ul className="space-y-2">
      {violations.length > 0 ? violations.map((v, i) => (
        ...
        <li className="text-[11px] text-slate-500 italic">No detailed violations recorded for this inspection.</li>
      )}
    </ul>
        ...
        <div className="absolute z-50 mt-1 w-full bg-slate-800 border border-slate-700 rounded-xl shadow-xl max-h-48 overflow-y-auto custom-scrollbar">
          {options.map(opt => (
            <label key={opt} className="flex items-center gap-2 px-3 py-1.5 hover:bg-slate-700 cursor-pointer text-sm text-slate-300">
              <input type="checkbox" checked={selected.includes(opt)} onChange={() => toggle(opt)} className="accent-indigo-500" /> {opt}
            </label>
          ))}
        </div>
      )}
...
// ... FilterLabel, FilterSelect, MinMaxInputs helper components same as before ...
...
export const CarrierSearch: React.FC<CarrierSearchProps> = ({ carriers, onSearch, isLoading, onNavigateToInsurance }) => {
  ...
  const applyFilters = () => {
    ...
    // ... rest of filtering logic mapped to CarrierFilters ...
    onSearch(f);
  };
  ...
  return (
    <div className="p-4 md:p-8 h-screen flex flex-col overflow-hidden relative selection:bg-indigo-500/30">
      {/* --- Keep Search Bar & Header Logic Same --- */}
          ...
          <div className="bg-slate-900 border-2 border-slate-700/50 w-full max-w-7xl max-h-[95vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col relative animate-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-8 bg-slate-900/40">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-slate-850/40 p-8 rounded-[2rem] border border-slate-800">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
                      <span className="text-[9px] text-slate-500 uppercase block mb-1">Hazmat Indicator</span>
                ...
                <div className="bg-slate-850/40 p-8 rounded-[2rem] border border-slate-800">
                  ...
                  <div className="space-y-4 max-h-[220px] overflow-y-auto custom-scrollbar pr-2">
                    {selectedCarrier.insurancePolicies?.map((p: any, i: number) => (
                      <div key={i} className="flex justify-between items-center bg-slate-900 p-4 rounded-2xl border border-slate-800 hover:border-indigo-500/30 transition-all">
                        <div>
                          <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{p.type}</p>
                          <p className="text-xs font-bold text-slate-200 truncate max-w-[200px] uppercase">{p.carrier}</p>
                        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* --- IMAGE STYLED INSPECTION SECTION --- */}
                <div className="bg-slate-850/40 p-8 rounded-[2rem] border border-slate-800 flex flex-col shadow-2xl">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <h4 className="text-xl font-black text-white uppercase tracking-tight flex items-center gap-3">
                        <Clock size={20} className="text-indigo-400" /> Inspections & Crashes
                      </h4>
                    </div>
                  ...
                  <div className="grid grid-cols-4 gap-3 mb-8">
                    <div className="bg-slate-900 border border-slate-800 p-3 rounded-xl shadow-sm">
                      ...
                      <span className="text-xl font-bold text-white leading-none">{selectedCarrier.inspections?.length || 0}</span>
                    </div>
                    <div className="bg-indigo-500/5 border-2 border-indigo-500/30 p-3 rounded-xl shadow-sm">
                    ...
                    <div className="bg-red-500/5 border-2 border-red-500/30 p-3 rounded-xl shadow-sm">
                      ...
                      <div key={i} className="group/item relative bg-slate-900/50 border border-slate-800 p-4 rounded-2xl hover:border-indigo-500/40 transition-all cursor-help">
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex gap-2 items-center">
                            <span className="bg-orange-500/10 text-orange-400 px-2 py-1 rounded-lg text-[9px] font-black border border-orange-500/20">
                              {insp.oosViolations + insp.driverViolations} Violations
                            </span>
                            {insp.oosViolations > 0 && (
                              <span className="bg-red-500/10 text-red-400 px-2 py-1 rounded-lg text-[9px] font-black border border-red-500/20 uppercase">OOS</span>
                            )}
                ...
                {/* Safety Information */}
                <div className="bg-slate-850/40 p-8 rounded-[2rem] border border-slate-800 flex flex-col shadow-2xl">
                  <h4 className="text-lg font-black text-white uppercase mb-8 flex items-center gap-2"><Shield size={20} className="text-indigo-400" /> Safety Information</h4>
                  <div className="bg-slate-900/50 p-6 rounded-3xl border border-slate-800 flex items-center gap-6 mb-6">
                    <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-400 border border-emerald-500/20 shadow-xl shadow-emerald-500/5">
                      <CheckCircle2 size={36} />
                    </div>
                    <div>
                      <p className="text-2xl font-black text-white uppercase tracking-tighter leading-none mb-1">{selectedCarrier.safetyRating || 'Satisfactory'}</p>
                      <p className="text-[10px] text-slate-500 font-bold uppercase">Last Audit: {selectedCarrier.safetyRatingDate || 'N/A'}</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 px-1">Enriched BASIC Scores</h5>
                    <div className="grid grid-cols-2 gap-3">
                      {selectedCarrier.basicScores?.map((score, i) => (
                        <div key={i} className="bg-slate-900 border border-slate-800 p-3 rounded-xl flex justify-between items-center">
                          <span className="text-[10px] text-slate-400 font-medium truncate max-w-[100px]">{score.category}</span>
                          <span className="text-xs font-black text-white">{score.measure}</span>
                        </div>
                      ))}
                    </div>
                  </div>
