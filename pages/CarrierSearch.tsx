...
import { Search, Eye, X, MapPin, Phone, Mail, Hash, Truck, Calendar, ShieldCheck, Download, ShieldAlert, Activity, Info, ExternalLink, CheckCircle2, Zap, Loader2, ChevronDown, ChevronUp } from 'lucide-react';
...
export const CarrierSearch: React.FC<CarrierSearchProps> = ({ carriers, onSearch, isLoading, onNavigateToInsurance }) => {
  ...
  const buildFilters = (): CarrierFilters => {
    ...
    // FIX: Safety filters now check for non-empty string to allow '0' as a valid input
    ...
    <div className="p-4 md:p-8 h-screen flex flex-col overflow-hidden relative selection:bg-indigo-500/30">
      ...
      <div className="flex gap-3 mb-4">
        <div className="relative group w-52 shrink-0">
          ...
          <input type="text" placeholder="Search MC#..." className="w-full bg-slate-850/80 border border-slate-700/50 rounded-2xl pl-9 pr-3 py-3 text-white text-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all shadow-xl" value={mcSearchTerm} onChange={(e) => setMcSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && applyFilters()} />
        </div>
        <div className="flex-1 relative group">
          ...
          <input type="text" placeholder="Search by Business Name..." className="w-full bg-slate-850/80 border border-slate-700/50 rounded-2xl pl-11 pr-4 py-3 text-white text-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all shadow-xl" value={nameSearchTerm} onChange={(e) => setNameSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && applyFilters()} />
        </div>
        ...
        <div className="mb-4 p-4 bg-slate-950/80 border border-slate-700/50 rounded-3xl overflow-y-auto max-h-[55vh] custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            <FilterGroup title="Motor Carrier" icon={<Truck size={12} />}>
              ...
              <div><FilterLabel>State</FilterLabel><MultiSelect options={US_STATES} selected={filters.state} onChange={v => setFilters(p => ({ ...p, state: v }))} placeholder="All" /></div>
              <div><FilterLabel>DOT Number</FilterLabel><input type="number" name="dot" value={filters.dot} onChange={handleFilterChange} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-indigo-500" /></div>
              ...
              <div><FilterLabel>Has Email</FilterLabel><FilterSelect name="hasEmail" value={filters.hasEmail} onChange={handleFilterChange} options={yesNoOptions} /></div>
              <div><FilterLabel>Has BOC-3</FilterLabel><FilterSelect name="hasBoc3" value={filters.hasBoc3} onChange={handleFilterChange} options={yesNoOptions} /></div>
              <div><FilterLabel>Company Rep. Available</FilterLabel><FilterSelect name="hasCompanyRep" value={filters.hasCompanyRep} onChange={handleFilterChange} options={yesNoOptions} /></div>
            </FilterGroup>
            <FilterGroup title="Carrier Operation" icon={<Activity size={12} />}>
              <div><FilterLabel>Classification</FilterLabel><MultiSelect options={OPERATION_CLASSIFICATIONS} selected={filters.classification} onChange={v => setFilters(p => ({ ...p, classification: v }))} placeholder="All" /></div>
              <div><FilterLabel>Carrier Operation</FilterLabel><MultiSelect options={CARRIER_OPERATIONS} selected={filters.carrierOperation} onChange={v => setFilters(p => ({ ...p, carrierOperation: v }))} placeholder="All" /></div>
              <div><FilterLabel>Hazmat</FilterLabel><FilterSelect name="hazmat" value={filters.hazmat} onChange={handleFilterChange} options={yesNoOptions} /></div>
              ...
              <div><FilterLabel>Cargo</FilterLabel><MultiSelect options={CARGO_TYPES} selected={filters.cargo} onChange={v => setFilters(p => ({ ...p, cargo: v }))} placeholder="All" /></div>
            </FilterGroup>
            <FilterGroup title="Insurance Policy" icon={<Shield size={12} />}>
              <div><FilterLabel>Required</FilterLabel><MultiSelect options={INSURANCE_REQUIRED_TYPES} selected={filters.insuranceRequired} onChange={v => setFilters(p => ({ ...p, insuranceRequired: v }))} placeholder="All" /></div>
              ...
              <div><FilterLabel>Has BIPD Insurance</FilterLabel><FilterSelect name="bipdOnFile" value={filters.bipdOnFile} onChange={handleFilterChange} options={yesNoNumOptions} /></div>
              ...
              <div><FilterLabel>Has Bond Insurance</FilterLabel><FilterSelect name="bondOnFile" value={filters.bondOnFile} onChange={handleFilterChange} options={yesNoNumOptions} /></div>
            </FilterGroup>
            <FilterGroup title="Safety" icon={<ShieldCheck size={12} />}>
              ...
              <div><FilterLabel>Injuries</FilterLabel><MinMaxInputs nameMin="injuriesMin" nameMax="injuriesMax" valueMin={filters.injuriesMin} valueMax={filters.injuriesMax} onChange={handleFilterChange} /></div>
              <div><FilterLabel>Fatalities</FilterLabel><MinMaxInputs nameMin="fatalitiesMin" nameMax="fatalitiesMax" valueMin={filters.fatalitiesMin} valueMax={filters.fatalitiesMax} onChange={handleFilterChange} /></div>
              <div><FilterLabel>Towaway</FilterLabel><MinMaxInputs nameMin="towawayMin" nameMax="towawayMax" valueMin={filters.towawayMin} valueMax={filters.towawayMax} onChange={handleFilterChange} /></div>
          ...
          <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-slate-800">
            <button onClick={resetAll} className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-sm font-bold transition-all border border-slate-700">Reset All</button>
            <button onClick={applyFilters} disabled={isLoading} className="px-8 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white rounded-xl text-sm font-bold transition-all shadow-lg shadow-indigo-500/20 flex items-center gap-2">{isLoading ? <><Loader2 size={14} className="animate-spin" /> Searching...</> : 'Apply Filters'}</button>
          </div>
        </div>
      )}
        ...
        <div className="overflow-auto custom-scrollbar flex-1">
          <table className="w-full text-left text-sm text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {!isLoading && carriers.length === 0 ? (<tr><td colSpan={5} className="p-16 text-center text-slate-600 italic">No results found matching your search criteria.</td></tr>) : (
                carriers.map((carrier, idx) => (
                  <tr key={idx} className="hover:bg-indigo-500/5 transition-colors group cursor-pointer" onClick={() => setSelectedDot(carrier.dotNumber)}>
                    ...
                    <td className="p-4 font-bold text-white group-hover:text-indigo-200 transition-colors truncate max-w-[250px]">{carrier.legalName}</td>
                    <td className="p-4 font-mono text-slate-400">{carrier.dotNumber}</td>
                    <td className="p-4"><span className={`text-[10px] px-2 py-0.5 rounded-full font-black tracking-tight border ${carrier.status?.includes('AUTHORIZED') && !carrier.status?.includes('NOT') ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>{carrier.status?.includes('AUTHORIZED') && !carrier.status?.includes('NOT') ? 'ACTIVE' : 'INACTIVE'}</span></td>
      ...
      {selectedCarrier && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-slate-900 border-2 border-slate-700/50 w-full max-w-7xl max-h-[95vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col relative animate-in zoom-in slide-in-from-bottom-4 duration-300">
            {/* MODIFIED HEADER: Height reduced by 30%, DBA removed, Emerald Green Badges added */}
            <div className="py-4 md:py-5 px-6 md:px-8 border-b border-slate-800 bg-slate-850/30 flex justify-between items-center">
              <div className="flex gap-4 md:gap-8 items-center">
                <div className="w-12 h-12 md:w-14 md:h-14 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center text-white shadow-xl shadow-indigo-500/10">
                  <Truck size={22} className="md:w-8 md:h-8" />
                </div>
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-4 mb-1.5">
                    <h2 className="text-xl md:text-2xl font-black text-white uppercase tracking-tighter truncate max-w-[300px] md:max-w-[700px] leading-tight">{selectedCarrier.legalName}</h2>
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border-2 ${selectedCarrier.status?.includes('NOT AUTHORIZED') ? 'bg-red-500/10 text-red-400 border-red-500/30' : 'bg-green-500/10 text-green-400 border-green-500/30'}`}>{selectedCarrier.status?.includes('NOT AUTHORIZED') ? 'Unauthorized' : 'Active Authority'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                     <div className="bg-[#10B981] rounded px-2 py-0.5 text-white text-[11px] font-bold uppercase shadow-sm">DOT {selectedCarrier.dotNumber}</div>
                     <div className="bg-[#10B981] rounded px-2 py-0.5 text-white text-[11px] font-bold uppercase shadow-sm">MC {selectedCarrier.mcNumber}</div>
                     <div className="bg-[#10B981] rounded px-2 py-0.5 text-white text-[11px] font-bold uppercase shadow-sm">Carrier</div>
                  </div>
                </div>
              </div>
              <button onClick={() => setSelectedDot(null)} className="p-3 text-slate-500 hover:text-white hover:bg-slate-800 rounded-2xl transition-all active:scale-75"><X size={28} /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar bg-slate-900/40">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-slate-850/60 p-6 rounded-3xl border border-slate-700/50 space-y-4 shadow-lg group">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3"><Phone size={16} className="text-indigo-400 shrink-0" /><div className="flex flex-col"><span className="text-[9px] text-slate-500 font-black uppercase">Phone</span><span className="text-base font-black text-white">{selectedCarrier.phone || 'N/A'}</span></div></div>
                    <div className="flex items-center gap-3"><Mail size={16} className="text-indigo-400 shrink-0" /><div className="flex flex-col overflow-hidden"><span className="text-[9px] text-slate-500 font-black uppercase">Email</span><span className="text-sm font-black text-indigo-300 truncate">{selectedCarrier.email || 'None Registered'}</span></div></div>
                    <div className="flex items-center gap-3"><MapPin size={16} className="text-indigo-400 shrink-0" /><div className="flex flex-col"><span className="text-[9px] text-slate-500 font-black uppercase">Location</span><span className="text-xs font-bold text-slate-300 leading-tight">{selectedCarrier.physicalAddress}</span></div></div>
                  </div>
                </div>
                <div className="bg-slate-850/60 p-6 rounded-3xl border border-slate-700/50 space-y-4 shadow-lg group">
                  ...
                  <div className="space-y-3">
                    <div className="bg-slate-900/50 p-3 rounded-2xl border border-slate-800 shadow-inner">
                      <div className="flex justify-between items-start mb-1"><span className="text-[9px] text-slate-500 font-black uppercase">MCS-150 Date</span>{calculateYearsInBusiness(selectedCarrier.mcs150Date) !== null && (<span className="text-[10px] font-black text-emerald-400 uppercase tracking-tighter">{calculateYearsInBusiness(selectedCarrier.mcs150Date)} Years in Biz</span>)}</div>
                      <span className="text-base font-black text-white">{selectedCarrier.mcs150Date || 'N/A'}</span>
                    </div>
                    <div className="bg-slate-900/50 p-3 rounded-2xl border border-slate-800 shadow-inner"><span className="text-[9px] text-slate-500 font-black uppercase block mb-1">Mileage / VMT</span><span className="text-sm font-bold text-slate-300">{selectedCarrier.mcs150Mileage || 'N/A'}</span></div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div className="bg-slate-850/40 p-8 rounded-[2rem] border border-slate-800 flex flex-col gap-6 shadow-2xl">
                  ...
                  <div className="space-y-6">
                    <div><h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Classifications</h5><p className="text-sm font-bold text-slate-200 bg-slate-900/50 p-3 rounded-xl border border-slate-800">{selectedCarrier.operationClassification?.join(', ') || 'N/A'}</p></div>
                    ...
                    <div><h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Cargo Carried</h5><div className="grid grid-cols-1 gap-2">{selectedCarrier.cargoCarried?.map((cargo, idx) => (<div key={idx} className="bg-slate-900/50 border border-slate-800 p-3 rounded-xl flex items-center gap-3"><Truck size={14} className="text-slate-600" /><span className="text-xs font-bold text-slate-300">{cargo}</span></div>))}</div></div>
                    <div className={`w-full py-4 rounded-2xl flex items-center justify-center font-black tracking-widest text-xs border-2 ${selectedCarrier.cargoCarried?.some(c => c.toLowerCase().includes('haz')) ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}`}>{selectedCarrier.cargoCarried?.some(c => c.toLowerCase().includes('haz')) ? 'HAZMAT INDICATOR: YES' : 'HAZMAT INDICATOR: NON-HAZMAT'}</div>
                    <div className="h-px bg-slate-800/50 my-2" />
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-900/50 border border-slate-800 p-3 rounded-xl flex flex-col items-center"><span className="text-[9px] text-slate-500 font-black uppercase mb-1">Power Units</span><span className="text-lg font-black text-white">{selectedCarrier.powerUnits || '0'}</span></div>
                      <div className="bg-slate-900/50 border border-slate-800 p-3 rounded-xl flex flex-col items-center"><span className="text-[9px] text-slate-500 font-black uppercase mb-1">Drivers</span><span className="text-lg font-black text-white">{selectedCarrier.drivers || '0'}</span></div>
                    </div>
                  </div>
                </div>
                <div className="bg-slate-850/40 p-8 rounded-[2rem] border border-slate-800 flex flex-col shadow-2xl">
                  <div className="flex items-center gap-3 mb-8"><ShieldCheck size={20} className="text-emerald-400" /><h4 className="text-xl font-black text-white uppercase tracking-tight">Verified L&I Filings</h4></div>
                  <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-2">
                    {selectedCarrier.insurancePolicies && selectedCarrier.insurancePolicies.length > 0 ? selectedCarrier.insurancePolicies.map((p: any, i: number) => (
                      <div key={i} className="bg-slate-900 p-6 rounded-[1.5rem] border border-slate-800 shadow-sm hover:border-indigo-500/30 transition-all">
                        <div className="flex justify-between items-start mb-4"><span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest border border-indigo-500/10 px-2 py-0.5 rounded-lg">{p.type} FILING</span><span className="text-xl font-black text-white">{p.coverageAmount}</span></div>
                        <p className="text-sm font-black text-slate-200 mb-4 truncate leading-tight uppercase">{p.carrier}</p>
                        <div className="flex justify-between items-center text-[10px] text-slate-500 font-mono pt-4 border-t border-slate-800/50"><span className="bg-slate-850 px-2 py-1 rounded">#{p.policyNumber}</span><span className="bg-slate-850 px-2 py-1 rounded">EFF: {p.effectiveDate}</span></div>
                      </div>
                    )) : <div className="flex flex-col items-center justify-center py-20 text-slate-700 text-center"><Info size={48} className="opacity-10 mb-4" /><p className="text-xs font-black uppercase text-slate-500 mb-2">No Filings Extracted</p></div>}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-slate-850/40 p-8 rounded-[2rem] border border-slate-800 flex flex-col gap-6 shadow-2xl relative">
                  <div className="flex items-center justify-between"><div className="flex items-center gap-3"><ShieldCheck size={20} className="text-indigo-400" /><h4 className="text-xl font-black text-white uppercase tracking-tight">Safety Info</h4></div><a href={`https://ai.fmcsa.dot.gov/SMS/Carrier/${selectedCarrier.dotNumber}/CompleteProfile.aspx`} target="_blank" className="text-[10px] font-bold text-indigo-400 flex items-center gap-1 hover:text-white"><ExternalLink size={12} /> FMCSA Source</a></div>
                    ...
                    <div className="space-y-8 animate-in fade-in duration-500">
                      <div className="flex justify-between items-start">
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400"><CheckCircle2 size={24} /></div>
                            <div><p className="text-sm font-black text-slate-200 uppercase">{selectedCarrier.safetyRating}</p><p className="text-[11px] text-slate-500 font-mono">ENRICHED: {selectedCarrier.safetyRatingDate}</p></div>
                          </div>
                        </div>
                        <div className="flex-1 max-w-[180px] space-y-4">
                          {selectedCarrier.oosRates?.map((oos, idx) => (
                            <div key={idx} className="space-y-1">
                              <div className="flex justify-between text-[10px] font-black uppercase"><span className="text-slate-500 truncate mr-2">{oos.type}</span><span className="text-emerald-400">{oos.rate}</span></div>
                              <div className="w-full bg-slate-800/50 rounded-full h-1 relative overflow-hidden"><div className="h-full bg-gradient-to-r from-emerald-600 to-emerald-400 rounded-full" style={{ width: `${parseFloat(oos.rate) || 0}%` }} /></div>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-x-8 gap-y-3">
                        {selectedCarrier.basicScores?.map((score, idx) => (
                          <div key={idx} className="flex justify-between items-center text-xs"><span className="text-slate-500 truncate">{score.category}</span><span className="text-slate-300 font-bold font-mono">{score.measure}</span></div>
                        ))}
                      </div>
                    </div>
                  ) : <div className="flex-1 flex flex-col items-center justify-center py-20 text-slate-700 text-center"><ShieldAlert size={48} className="opacity-20 text-indigo-500" /><p className="text-xs font-black text-slate-500 uppercase">Not Enriched</p></div>}
                </div>
                ...
                <div className="bg-white rounded-[2rem] p-8 flex flex-col shadow-xl text-slate-900">
                  ...
                  <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
                    <button onClick={() => setActiveTab('inspections')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'inspections' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}>Inspections</button>
                    <button onClick={() => setActiveTab('crashes')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'crashes' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}>Crashes</button>
                  </div>
                  <div className="grid grid-cols-4 gap-4 mb-6">
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center"><span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Total</span><span className="text-xl font-black">{activeTab === 'inspections' ? selectedCarrier.inspections?.length || 0 : selectedCarrier.crashes?.length || 0}</span></div>
                    <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100 text-center"><span className="text-[10px] font-bold text-blue-400 uppercase block mb-1">Viol.</span><span className="text-xl font-black text-blue-600">{selectedCarrier.inspections?.reduce((acc, curr) => acc + (curr.violationList?.length || 0), 0) || 0}</span></div>
                    <div className="bg-red-50 p-4 rounded-2xl border border-red-100 text-center"><span className="text-[10px] font-bold text-red-400 uppercase block mb-1">OOS</span><span className="text-xl font-black text-red-600">{selectedCarrier.inspections?.reduce((acc, curr) => acc + (curr.oosViolations || 0), 0) || 0}</span></div>
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center"><span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Crashes</span><span className="text-xl font-black">{selectedCarrier.crashes?.length || 0}</span></div>
                  </div>
                  <div className="space-y-3 max-h-[350px] overflow-y-auto custom-scrollbar pr-2">
                    {activeTab === 'inspections' ? selectedCarrier.inspections?.map((insp, i) => (
                      <div key={i} className="border border-slate-100 rounded-2xl overflow-hidden">
                        <div className="p-4 flex justify-between items-center cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => setExpandedInspection(expandedInspection === insp.reportNumber ? null : insp.reportNumber)}>
                          <div><p className="text-sm font-black">{insp.date}</p><p className="text-[11px] text-slate-500">{insp.location}</p></div>
                          <div className="flex items-center gap-2">{insp.oosViolations > 0 && <span className="bg-red-100 text-red-700 px-2 py-0.5 rounded-lg text-[10px] font-black">OOS</span>}{expandedInspection === insp.reportNumber ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</div>
                        </div>
                        {expandedInspection === insp.reportNumber && (
                          <div className="px-4 pb-4 pt-4 border-t border-slate-100 bg-[#F8F9FA]">
                            <div className="grid grid-cols-2 gap-4 text-[12px]">
                              <div><span className="text-slate-500">Report #:</span> <span className="font-bold">{insp.reportNumber}</span></div>
                              <div><span className="text-slate-500">OOS Viol:</span> <span className="font-bold">{insp.oosViolations}</span></div>
                            </div>
                            {insp.violationList?.length > 0 && (
                              <div className="mt-4 space-y-2 border-t border-slate-200 pt-3">
                                {insp.violationList.map((v: any, vi: number) => (
                                  <div key={vi} className="bg-white p-2 rounded-lg border border-slate-200 text-[11px]"><span className="text-blue-600 font-bold">{v.label}</span><p className="text-slate-600">{v.description}</p></div>
                                ))}
                              </div>
                            )}
                          </div>
                      ...
                      <div key={i} className="p-4 border border-slate-100 rounded-2xl flex justify-between items-center"><div><p className="text-sm font-black">{crash.date}</p><p className="text-[11px] text-slate-500">{crash.state} | {crash.number}</p></div><div className="flex gap-2">{parseInt(crash.fatal) > 0 && <span className="bg-red-600 text-white px-2 py-0.5 rounded-lg text-[10px] font-black">FATAL</span>}</div></div>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {/* MODIFIED FOOTER: Minimum height and only Close View button */}
            <div className="py-2.5 px-6 bg-slate-950/75 border-t border-slate-800 flex justify-end">
              <button onClick={() => setSelectedDot(null)} className="px-6 py-1.5 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-bold border border-slate-700 active:scale-95 transition-all">Close View</button>
            </div>
          </div>
        </div>
      )}
