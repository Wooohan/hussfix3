...
const MAX_RETRIES = 0; // Total 3 attempts per MC
...
export const Scraper: React.FC<ScraperProps> = ({ user, onUpdateUsage, onNewCarriers, onUpgrade, onFinish }) => {
  ...
  const toggleRun = () => {
    if (isRunning) {
      ...
      setLogs(prev => [...prev, `🚀 Engine Started | Concurrency: ${CONCURRENCY_LIMIT} | Retries: ${MAX_RETRIES}`]);
      setScrapedData([]);
      setProgress(0);
      setDbSaveCount(0);
    ...
    const worker = async (mc: string) => {
      ...
      while (attempt <= MAX_RETRIES && isRunningRef.current) {
        ...
        try {
          if (config.useMockData) {
            ...
            // Validate that we didn't get an empty "N/A" object
            if (baseCarrier && baseCarrier.dotNumber && baseCarrier.legalName !== 'N/A') {
          ...
          if (newData) {
            ...
            throw new Error("Data returned N/A");
          }
        } catch (e) {
          attempt++;
          if (attempt <= MAX_RETRIES) {
            const backoff = attempt * 1500; // 1.5s, then 3s delay
            setLogs(prev => [...prev, `[Retry ${attempt}] MC ${mc} failed. Cooling down ${backoff}ms...`]);
            await sleep(backoff);
          }
        }
      }
      ...
      if (newData) {
        ...
        if (matchesFilter) {
          ...
          setLogs(prev => [...prev, `[Success] MC ${mc} | Rating: ${newData!.safetyRating} | ${finalLatency}ms`]);
          onUpdateUsage(1);
        }
      } else {
        setLogs(prev => [...prev, `[Failed] MC ${mc} reached max retries. Server blocked request.`]);
      }

      // Variable delay between 0.5s and 1.2s to simulate human behavior
      await sleep(Math.floor(Math.random() * 700) + 500);
      completed++;
            ...
            <div className="flex-[1] bg-slate-950 border border-slate-800 rounded-3xl p-5 font-mono text-[11px] overflow-y-auto relative custom-scrollbar">
              ...
              <div className="space-y-1">
                {logs.map((log, i) => {
                  ...
                  return (
                    <div key={i} className={`flex gap-3 ${isSuccess ? 'text-indigo-400' : isRetry ? 'text-yellow-500' : isFailed ? 'text-red-400' : 'text-slate-500'}`}>
